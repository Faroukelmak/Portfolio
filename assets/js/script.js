...
`
  ,
    `
    <div style="max-width: 800px; margin: 60px auto; font-family: Arial, sans-serif; color: #333; line-height: 1.7; padding: 30px; background-color: #f8f8f8; box-shadow: 0 0 15px rgba(0,0,0,0.1); border-radius: 12px;"> <section> <h2 style="font-size: 2em; color: #0077B6; margin-bottom: 25px; border-bottom: 3px solid #0077B6; padding-bottom: 10px;">Text Summarizer – Fine-Tuning PEGASUS et Pipeline MLOps Industriel 🧠</h2> <img src="./assets/images_me/textsummarizer_arch.png" alt="Architecture du pipeline MLOps pour le Text Summarizer" style="display: block; margin: 0 auto 1.5rem auto; max-width: 100%; border-radius: 8px; border: 1px solid #ddd;" />

<section> <h3 style="color: #00A896;">🎯 Introduction & Objectifs Techniques</h3> <p> Ce projet est une démonstration complète de mes compétences en NLP, Fine-Tuning et MLOps. Il consiste à développer un système de <strong>Synthèse de Texte Abstraite</strong> de pointe. </p> <ul> <li>Modèle : Fine-tuning du modèle PEGASUS (google/pegasus-cnn_dailymail), un modèle SOTA Seq2Seq de type Transformer.</li> <li>Tâche Spécifique : Spécialisation du modèle sur le dataset SAMSum pour le résumé de conversations naturelles (une tâche à haute valeur ajoutée).</li> <li>Objectif MLOps : Concevoir une pipeline MLOps complète, dockerisée, modulaire et déployée via une API FastAPI pour simuler un environnement de production.</li> </ul> </section>

<section> <h3 style="color: #00A896; margin-top: 30px;">⚙️ Méthodologie : Déploiement de la Pipeline MLOps</h3>

<h4 style="margin-top: 15px; color: #333;">1. Ingestion et Préparation des Données (Data Engineering)</h4> <p> Utilisation de Pandas et des outils Hugging Face pour traiter le dataset SAMSum. </p> <ul> <li>Prétraitement : Chargement, nettoyage et tokenisation du dataset pour générer les input_ids, attention_mask et les target labels (résumés de référence).</li> <li>Séparation : Split train / validation pour une évaluation rigoureuse en dehors du set d'entraînement.</li> </ul>

<h4 style="margin-top: 25px; color: #333;">2. Fine-Tuning de Modèle SOTA (Deep Learning)</h4> <p> Adaptation du modèle PEGASUS à la sémantique des dialogues. L'entraînement est géré par la librairie PyTorch. </p>

<ul> <li>Entraînement : Utilisation de la classe Trainer de Hugging Face pour une gestion efficace de l'entraînement.</li> <li>Optimisation : Mise en œuvre d’un scheduler linéaire et de l'optimiseur AdamW.</li> <li>Évaluation : Calcul des métriques ROUGE à chaque checkpoint pour le suivi des performances.</li> </ul>

<h4 style="margin-top: 25px; color: #333;">3. Pipeline MLOps et Évaluation des Performances</h4> <p>Structure modulaire garantissant la traçabilité et la reproductibilité du modèle final :</p>

<ul> <li>Ingestion & Transformation : Étapes initiales de préparation des données.</li> <li>Training & Évaluation : Entraînement et calcul des scores ROUGE pour la sélection du meilleur modèle.</li> <li>Prédiction : Implémentation du script de génération de texte utilisant l'algorithme de Beam Search.</li> <li>Sauvegarde : Stockage du meilleur checkpoint pour le déploiement.</li> </ul>

<h4 style="margin-top: 25px; color: #333;">4. API de Déploiement et Conteneurisation</h4> <p> Le modèle est exposé pour la consommation en temps réel. </p> <img src="./assets/images_me/textsummarizer_api.png" alt="Interface de l'API FastAPI" style="display: block; margin: 0 auto 1.5rem auto; max-width: 100%; border-radius: 8px; border: 1px solid #ddd;" /> <ul> <li>API Performante : Développement d'une API avec FastAPI et Uvicorn pour des requêtes concurrentes rapides.</li> <li>Schémas Pydantic : Définition stricte des entrées/sorties pour une qualité de code et une documentation automatiques.</li> <li>Dockerisation : Encapsulation complète du service dans un conteneur Docker pour garantir la portabilité et un déploiement fiable.</li> </ul> </section>

<section> <h3 style="color: #00A896; margin-top: 30px;">⭐ Outils et Technologies Clés Utilisés</h3> <table style="width: 100%; border-collapse: collapse; margin-top: 20px;"> <thead> <tr style="background-color: #E6F5FF;"> <th style="text-align: left; padding: 12px; border: 1px solid #ddd; color: #333;">Catégorie</th> <th style="text-align: left; padding: 12px; border: 1px solid #ddd; color: #333;">Outils / Technologies</th> </tr> </thead> <tbody> <tr> <td style="padding: 12px; border: 1px solid #ddd;">Deep Learning & NLP</td> <td style="padding: 12px; border: 1px solid #ddd;">PyTorch (torch), PEGASUS, Hugging Face Transformers</td> </tr> <tr> <td style="padding: 12px; border: 1px solid #ddd;">Évaluation & Métriques</td> <td style="padding: 12px; border: 1px solid #ddd;">Hugging Face evaluate pour ROUGE-1, ROUGE-2, ROUGE-L</td> </tr> <tr> <td style="padding: 12px; border: 1px solid #ddd;">Déploiement API & Infra</td> <td style="padding: 12px; border: 1px solid #ddd;">FastAPI, Uvicorn, Docker (conteneurisation)</td> </tr> <tr> <td style="padding: 12px; border: 1px solid #ddd;">Data Engineering</td> <td style="padding: 12px; border: 1px solid #ddd;">Pandas (prétraitement des données)</td> </tr> <tr> <td style="padding: 12px; border: 1px solid #ddd;">Architecture</td> <td style="padding: 12px; border: 1px solid #ddd;">Architecture Modulaire (Python), Pipeline MLOps</td> </tr> </tbody> </table> </section> </section> </div>
`
  ,
